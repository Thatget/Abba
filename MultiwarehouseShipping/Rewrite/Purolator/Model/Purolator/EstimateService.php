...
namespace MW\MultiwarehouseShipping\Rewrite\Purolator\Model\Purolator;
...
class EstimateService extends \Purolator\Shipping\Model\Purolator\EstimateService
{
    ...
    public function __construct(
        ...
        parent::__construct($helper, $storeManager, $scopeConfig, $regionInterfaceFactory, $regionResourceModel);
    }
    ...
    )
    {
        ...
        if ($rateRequest->getOrigPostcode()) {
        	$request->Shipment->SenderInformation->Address->City = $rateRequest->getOrigCity();
	        $region = $this->regionInterfaceFactory->create();
	        $this->regionResourceModel->load(
	            $region,
	            $rateRequest->getOrigRegionId(),
	            'region_id'
	        );
	        $request->Shipment->SenderInformation->Address->Province = $region->getCode();
	        $request->Shipment->SenderInformation->Address->Country = $rateRequest->getOrigCountry();
	        $request->Shipment->SenderInformation->Address->PostalCode = $rateRequest->getOrigPostcode();
        } else {
        	$request->Shipment->SenderInformation->Address->City = $this->scopeConfig->getValue('shipping/origin/city', ScopeInterface::SCOPE_STORE);
	        $region = $this->regionInterfaceFactory->create();
	        $this->regionResourceModel->load(
	            $region,
	            $this->scopeConfig->getValue('shipping/origin/region_id', ScopeInterface::SCOPE_STORE),
	            'region_id'
	        );
	        $request->Shipment->SenderInformation->Address->Province = $region->getCode();
	        $request->Shipment->SenderInformation->Address->Country = $this->scopeConfig->getValue('shipping/origin/country_id', ScopeInterface::SCOPE_STORE);
